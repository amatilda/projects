...
typedef struct				s_process_42sh//Отвечает за все что связанно с процессами
{
	...
	pid_t					pid_not_fork;//pid - для перенаправлений общий что бы отслеживавать
...
typedef struct				s_task_main//Хранит все что связанно с переменной среды HOME
{
	pthread_t				thread;
	pthread_mutex_t			mutex;
	size_t					count_read;
	void					*path_file;//Указатель на файл канфигурации
	void					*task_list;//Указатель для таскменеджера
	size_t					b_init;//Указывает иницилизируеться при старте 1 или просто через лоад 0
	size_t					b_delete;//Указывает нужно игнорить перезапуск 1 или нет 0
	size_t					b_reload;//Указывает что сейчас нужно сделать reaload 1 или нет 0
	size_t					b_cmd;//Указывает что сейчас выполняеться команда 1 или нет
}							t_task_main;

typedef struct				s_glb_list
{
	struct s_glb_list		*prev;// должно быть зануленно
	struct s_glb_list		*next;// должно быть зануленно/
	char					name[1];
}							t_glb_list;
...
typedef struct				s_main_42sh//Главная структура проекта
{
	...
	void					*draynor;//Указатель на хранилеще
	t_task_main				task;
	t_dq_42sh				dq;
	t_process_42sh			pr;
	t_in_main_42sh			in;
