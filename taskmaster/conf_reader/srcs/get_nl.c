...
/*   Updated: 2020/06/18 16:03:22 by wstygg           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		fn_while(t_get_next_line *gnl)
{
	...
	if (gnl->interval == 0 && gnl->line_count != 0)
		return (0);
	gnl->lp_prev = (char *)my_malloc(gnl->interval + gnl->line_count + 1);
	if (gnl->lp_prev == 0)
	{
		gnl->count = -1;
		return (0);
	...
	if (gnl->interval != 0)
	{
		if (gnl->line_count != 0)
		{
			my_memcpy(gnl->lp_prev, gnl->line, gnl->line_count);
			free(gnl->line);
		}
		my_memcpy(gnl->lp_prev + gnl->line_count, gnl->buffer, gnl->interval);
							...
							char **lp)
{
	...
	if (gnl->lp_cmp != 0 && (size_t)gnl->count > gnl->interval)
	{
		...
		gnl->lp_cmp = my_malloc(gnl->tmp + sizeof(size_t));
		if (gnl->lp_cmp == 0)
		...
		*((size_t *)gnl->lp_cmp) = gnl->tmp;
		...
		my_memcpy(gnl->lp_cmp, &gnl->buffer[gnl->interval], gnl->tmp);
	}
	else if (lp[fd] != 0)
	{
		free(lp[fd]);
...
int				get_nl(const int fd, char **line)
{
	...
	if (lp[fd] != 0)
	{
		...
		my_memcpy(gnl.buffer, lp[fd] + sizeof(size_t), gnl.count);
	}
	else
