...
register t_jobs_42sh *jobs)//Закрываем все right pipe  и не нужный на чтение с стороны left
{
...
register t_jobs_42sh *jobs)//Закрываем все left pipe  и не нужный на запись с стороны right
{
	register t_pipe_42sh		*pipe;
	register t_pipe_42sh		*pipe_end;
	register int				*fds;
