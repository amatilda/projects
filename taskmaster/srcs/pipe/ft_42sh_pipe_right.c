...
register t_jobs_42sh *jobs, register int fd, t_pipe_search_in_42sh *in)
{
	...
	if (ft_42sh_pipe_find_count(&jobs->pipe[0], &jobs->pipe[jobs->n], fd) == 1)
		return (1);//Провераем если перенаправлекние fd один раз происходит а не множества раз тогда делаем как в баше простым dup2
	if ((pid = fork()) == 0)
	{
		ft_42sh_signal_default(array, jobs);
		setpgid(0, array->pr.pid_fork);//Добавим в группу процессов в которой запускаемся
		ft_42sh_pipe_close_fd_right(array, jobs);//Освободим fd и pipe right иначе подвиснит// Что бы процесс завершался
