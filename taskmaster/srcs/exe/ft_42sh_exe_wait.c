...
register t_jobs_42sh *jobs, t_jobs_42sh *jobs_cl)//Функция для работы  не в валгринде
{
	...
	while (0xFF)
	{
		if (tmp->pid_view != 0 || array->pr.pid_not_fork != 0)//Перебираем pid_view так как если запущен процесс пид тут всегда храниться и полюбому нужно забрать записанные данные в пайп
		{
			if (read(array->pr.even_fds[PIPE_READ_42SH], buff, 1) == -1)
				ft_42sh_exit(E_PIPE_CODE_42SH, __FILE__, __func__, __LINE__);
			array->pr.pid_not_fork = 0;//Это если грцпповое перенаправление но бултин без форка вызван
			break ;
		}
...
register t_jobs_42sh *jobs, t_jobs_42sh *jobs_cl)//Функция для работы в валгринде
{
	...
	if ((pid = array->pr.pid_not_fork) != 0)//Тогда это бултин не в форке но имеющий множественные перенаправления поэтому ждем тут
	{
